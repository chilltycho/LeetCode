/*给定单词数组和长度maxWidth，重新排版单词，使每行恰好有maxWidth个字符，且左右两端对齐
文本最后一行应左对齐，单词间不插入额外空格*/
#include <iostream>
#include <vector>
using namespace std;
string fillWords(vector<string> &words, int bg, int ed, int maxWidth,
  ...
  int spaceCount =
      maxWidth + 1 - wordCount; // 除去每个单词尾部空格， + 1 是最后一个单词的尾部空格的特殊处理
                                // 
  for (int i = bg; i <= ed; i++) {
  ...
  int spaceAvg =
      (wordCount == 1) ? 1 : spaceCount / (wordCount - 1); // 额外空格的平均值
  int spaceExtra =
      (wordCount == 1) ? 0 : spaceCount % (wordCount - 1); // 额外空格的余数
