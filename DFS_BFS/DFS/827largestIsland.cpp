...
int dfs_1(vector<vector<int>> &grid, int r, int c, int index) {
  if (inArea(grid, r, c) && grid[r][c] == 1) {
    grid[r][c] = index;
    return 1 + dfs_1(grid, r - 1, c, index) + dfs_1(grid, r + 1, c, index) +
           dfs_1(grid, r, c - 1, index) + dfs_1(grid, r, c + 1, index);
  } else
    return 0;
}
...
int main() {
  ...
  // cout << largestIsland(vii1) << ' ' << largestIsland(vii2) << ' ' <<
  // largestIsland(vii3) << endl;
  cout << largestIsLand(vii1) << ' ' << largestIsLand(vii2) << ' '
       << largestIsLand(vii3) << endl;
}