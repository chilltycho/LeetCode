...
using namespace std;
/*
198. 打劫农舍问题：每个房子内有一定现金，不能偷相邻两间，求能偷最大金额。
...
f(i,j)=max(f(i-1,j),f(i,j-1)) if s[i-1]!=t[j-1]
3.确定dp数组的计算顺序
dp数组大小：0<=i<=m,0<=j<=m故大小为m*n
base case和源问题在dp数组中位置
dp数组中子问题依赖方向：dp[i][j]依赖于f[i-1,j] f[i,j-1]
f[i-1,j-1]故为从左到右、从上到下
*/
...
vector<int> rob_backup(vector<int> &nums) {
  ...
  dp[0] = 0, dp[1] = nums[0];
  back[1] =
      -2; // 若back[k]=-1表示dp[k]由dp[k-1]计算而来，若back[k]=-2表示dp[k]由dp[k-2]计算而来
  for (int k = 2; k <= n; k++) {
