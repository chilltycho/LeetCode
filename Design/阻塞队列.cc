...
void *consumer(void *)
{
    ...
    {
        pthread_mutex_lock(&mtx);
        while (avail == 0) // 必须while不能if，wait返回时不能确定条件状态，应该立即重新检查判断条件。wait返回时，可能多个线程等待，其他线程可能先获取了avail。
        {
            pthread_cond_wait(&cond, &mtx); // 解锁互斥量（让其他线程访问avail）并阻塞
        }
        while (avail > 0) // 被唤醒并加锁，因为该线程会立即访问avail
        {
