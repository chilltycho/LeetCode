...
bool isBalanced_down2head(TreeNode *root) { return dfs_down2head(root) != -1; }

int depth_head2down(TreeNode *root) {
  if (root == nullptr)
    return 0;
  return max(depth_head2down(root->left), depth_head2down(root->right)) + 1;
}
...
bool isBalanced_head2down(TreeNode *root) {
  if (root == nullptr)
    return true;
  return abs(depth_head2down(root->left) - depth_head2down(root->right)) <= 1 &&
         isBalanced_head2down(root->right) && isBalanced_head2down(root->left);
}
int main() {
