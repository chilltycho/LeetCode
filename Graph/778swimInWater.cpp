...
bool dfs(vector<vector<int>> &grid, vector<vector<int>> &visited,
         vector<int> &dir, int waterHeight, int row, int col, int n) {
  visited[row][col] = 1;
  for (int i = 0; i < 4; i++) {
    int r = row + dir[i];
    int c = col + dir[i + 1];
...
int main() {
  ...
  vector<vector<int>> vi1{{0, 1, 2, 3, 4},
                          {24, 23, 22, 21, 5},
                          {12, 13, 14, 15, 16},
                          {11, 17, 18, 19, 20},
                          {10, 9, 8, 7, 6}};
  ...
  vector<vector<int>> vi2{
      {0, 1, 4}, {2, 8, 7}, {3, 6, 5}}; //最优路径并非start周围最低点
